...
class DeleteModel(object):
    ...
    @property
    def GoodsId(self):return self.__goodsId
...
#     @exception_show_class
    ...
    def CheckState(self,operType,productionId):
        ...
        if _model:
            ...
            return dataUtil.ResError("不存在数据行")
    ...
    def __check_stock_id(self,userId):
        ...
        @param userId: 供应商ID号 
        """
        has_stock = dataUtil.getModel(_getOper('stock'),support_id=userId)
        return has_stock
    ...
    def CheckDeleteUser(self,userId,roleType):
        ...
        if roleType == "supporter":
            state = self.__check_stock_id(userId)
        else:
            state = self.__check_operate_id(userId, roleType)
        return state
