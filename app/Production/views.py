...
def __add_model(form, _oper, errorMsg=u"失败"):
    ...
    return _dataUtil.ResErrorJson(errorMsg)
...
def stock():
    ...
    if _initPageUtil.FormError is not None:
        return _dataUtil.ResErrorJson(_initPageUtil.FormError)
...
def _add_sell(form, stock_num):
    ...
    if not (state and model):
        _delete_moeny(form.money_id.data)
        return _dataUtil.ResErrorJson(u"销售信息的数据格式不正确:%s" % model)

    stock = GetStock(model.stock_id, 0)
    if not stock:
        _delete_moeny(form.money_id.data)
...
@json_return
def load_back_sells():
    data = _searchUtil.LoadBackSell(operator_id=current_user.id,
...
def roll_back():
    ...
    if state:
        if form.number.data > _getOper("sell").get_snap_number(form.sell_id.data):
...
def modify_cell():
    ...
    return _modifyCellUtil.UpateModel(_datas)
...
def delete_check_state():
    return _deleteModelUtil.CheckState(request.form.get("operType"),
...
def delete_production(operType):
    return _deleteModelUtil.DeleteModels(operType, request.form.get('production_id'))
...
def load_datagrid(operType):
    return render_template('Production/table_datagrid.html', operType=operType,
                           operTypeIndex=get_operTypeIndex(operType))
