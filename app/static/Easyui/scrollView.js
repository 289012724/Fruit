...
var scrollview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	onBeforeRender: function(target){
		...
		var dc = state.dc;
		var view = this;
		var _filter = $(target).data('filterObj');
		if(_filter.filterSource == null || 
				!_filter.filterSource.rows || 
				_filter.filterSource.rows.lenght==0 ){
				_filter.init({total:state.data.total,
							  rows:state.data.rows,
							  footer:state.data.footer});
				state.data.firstRows = state.data.rows;
		}
		state.data.firstRows = _filter.filterFeildData.rows;
		state.data.rows = [];
