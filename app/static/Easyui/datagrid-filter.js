...
(function ($) {
    ...
    function _s(type, field) {
        if (type == 1) {
            return function (one, two) {
                _one = one[field];
                _two = two[field];
                if (_one < _two) return -1;
                if (_one == _two) return 0;
                return 1;
            }
        } else {
            return function (one, two) {
    ...
    function _u3(fun, fun1) {
        var _filter = $(datagrid).data("filterObj");
        var _f = _filter.field;
        var _do = $.extend({}, _filter.filterFeildData);
        var _ds = _do.rows;
        var _v = _g1("input.excel_prompt_input").combobox("getText");
        var _v1 = _g1("input.excel_prompt_input1").combobox("getText");
        var _r2 = _g1("input.excel_checkbox1").attr("checked");
        var _rs = [];
        if (_v || _v1) {
            for (var _1 in _ds) {
                var _c = _ds[_1][_f];
                var _f1 = fun ? fun(String(_c), String(_v)) : true;
                var _f2 = fun1 ? fun1(String(_c), String(_v1)) : true;
                if (_r2) {
                    var _ok = _f1 || _f2;
                } else {
                    var _ok = _f1 && _f2;
    ...
    function _d1(opt) {
        var _filter = $(datagrid).data("filterObj");
        var data = _filter.filterFeildData;
        var field = _filter.field;
        var _do = $.extend({}, data);
     ...
     */
    function _u2(opt) {
        var _filter = $(datagrid).data("filterObj").init();
        var _data = _filter.filterFeildData;
        var field = _filter.field;
        var _ok = _d3(_data, field);
        var _inpnut = $(datagrid).data("filter_dialog")
        ...
        if (_div) {
            ...
            $(".excel_button").linkbutton({
                onClick: function () {
                    ...
                    if ($(this).attr('data') == 1) {
                        var opt = _g1("input.excel_user_opt").combobox("getValue");
                        var opt2 = _g1("input.excel_user_opt1").combobox("getValue");
                        if (operators[opt] || operators[opt2]) {
                            $(datagrid).datagrid('loadData', _u3(operators[opt], operators[opt2]));
                        } else {
     ...
     */
    function _d2(target) {
        var _div = $('<div class="execel_prompt"></div>');
        _div.css("padding", "10px 5px");

        //-------------------------------------------------------------
        var _text = $("<label>显示行:</label>");
        _text.css("padding", "1px");
        _text.appendTo(_div);

        //-------------------------------------------------------------
        var _type = $('<div class="excel_prompt_opt">问题描述<br/></div>');
        _type.appendTo(_div);
        var _userSelect = $("<input class='excel_user_opt'></input>");
        _userSelect.appendTo(_type);
        var _input = $("<input class='excel_prompt_input'></input>");
        _input.appendTo(_type);

        //-------------------------------------------------------------
        var _type1 = $('<div class="excel_prompt_opt" style="margin-top:10px"></div>');
        _type1.appendTo(_div);
        var _radio1 = $('<input type="radio" value="1" class="excel_checkbox0" >与</input>');
        _radio1.appendTo(_type1);
        _radio1.attr("checked", "checked");
        var _radio2 = $('<input type="radio" value="2" class="excel_checkbox1" >或</input>');
        ...
        //-------------------------------------------------------------
        var _type2 = $('<div class="excel_prompt_opt" style="margin-top:10px"></div>');
        _type2.appendTo(_div);
        var _userSelect1 = $("<input class='excel_user_opt1'></input>");
        _userSelect1.appendTo(_type2);
        var _input2 = $("<input class='excel_prompt_input1'></input>");
        _input2.appendTo(_type2);

        //-------------------------------------------------------------
        var b_div = $("<div class='filter_button' id='filter_button'></div>")
        b_div.css("marginTop", "-10px");

        //-------------------------------------------------------------
        var bt1 = $("<a href='#' class='excel_button'  data=1>确定</a>");
        var bt2 = $("<a href='#' class='excel_button'  data=0>取消</a>");
        bt1.appendTo(b_div);
        bt2.appendTo(b_div);
    ...
    function filter_obj() {
        ...
        function add(data, func_s) {
            var func = _o.filterSource.rows[func_s];
            func(data);
            var func = _o.filterFeildData.rows[func_s];
            func(data);
        };
        _o.add_row = function (data, index) {
            ...
            if (_o.filterSource.rows.length == 0) {
                return add(data, "push");
            }
        ...
        function _set_row(data, func) {
            if (_o.filterSource == null)return;
            if ($.isArray(data)) data = data[0];
            setTimeout(func(data.id, _o.filterSource, data), 1);
            setTimeout(func(data.id, _o.filterFeildData, data), 1);
        }

        _o.delete_row = function (data) {
            _set_row(data, function (id, _datas) {
        ...
        _o.modify_row = function (data) {
            _set_row(data, function (id, key, data) {
                var _datas = _o[key].rows;
                for (var ids in _datas) {
                    if (_datas[ids].id == id) {
                        _datas[ids] = data;
                    }
    ...
    function init(target) {
        ...
        function _g4(_menu) {
            var _s = [];
            var nodes = _menu.menu('excel_tree').tree('getChecked');
            for (var ids in nodes) {
        ...
        function _change_checked(node) {
            ...
            var _check = $(node.target).find("span.tree-checkbox");
            if (node.checked) {
        ...
        function _e1(_menu) {
            var _tree = _menu.menu("excel_tree");
            _tree.tree({
        ...
        function _mf1(event) {
            ...
            var _selects = _g4(_menu);
            if (_selects.length == 0) {
                _alert("提示", "未选中任何数据,请重新操作", "error");
                return $(menu).menu("hide");
            }

            var _field = _filter.field;
            var data = _filter.filterSource;
            var _total = data.rows;
            var _ok = [];

            for (var cell in _total) {
                var _c = _total[cell][_field];
            ...
            if (_ok.length == 0) {
                ...
                return $(_menu).menu('hide');
            }

            var _data = {total: _ok.length, rows: _ok, footer: data.footer};
        ...
        function _a1(target, menu) {
            ...
            for (ids in _labels) {
                menu.menu('appendItem', _labels[ids])
            }
        ...
        function _bdm(e) {
            ...
            var _parent = $(this).find('.datagrid-cell')[0];
...
(function ($) {
    ...
    var _e1 = function (e) {
        ...
        var _do = _tree.data("tree_data");
        var _input = _gp1(menu);
        value = $(_input).val();
        var _ds = [];
        if (value) {
            for (ids in _do) {
    ...
    var _tr1 = function (jq) {
        return jq.menu('excel').find("div.excel-menu .excel-menu-tree");
    };
    ...
    var _p1 = function (parent) {
        var _input = $("<input class='excel-menu-input' placeholder='搜索(按回车确认)' style='width:100%'></input>");
    ...
    var _ef = function (menu, data) {
        menu = $(menu);
        var _opt = menu.menu("options");
        var _ht = data.height || _opt.excelMenuTreeHeight;
        menu.css("width", menu.width() + _opt.excelMenuWidth);
        var _excel = menu.menu("findItem", "__excel__filter");
    ...
    $.fn.menu.methods.appendItem = function (menu, data) {
        menu.each(function () {
            ...
            if (data._type && data._type == 'excel') _ef(menu, data);
            _r1(this);
        });
    };
