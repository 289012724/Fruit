...
class InitePage(object):
    ...
    @UserId.setter
    @types(long,True)
    def UserId(self,value):self.__userId = value
    
    @property
    def OperateType(self):return self.__operateType
    
    @OperateType.setter
    def OperateType(self,value):self.__operateType = value
    ...
    def _add_user(self,form):
        ...
        if depart_id:
            ...
            if st and model:
                ...
                if state: index, flag, model = 1, u'成功', modelList[0]
        else:
            index, flag, model = 2, u'部门不存在，请先联系管理员创建该部门', None
        return index, flag, model
    ...
    def __get_register_page(self,RegistForm):
        ...
        def user():
            ...
            RegistForm.department_id.choices = choise
            return RegistForm,choise
        if self.OperateType == "department":
            return department()
        else:
        ...
        if self.CheckFrom(form):
            if roleType == "department":
                ...
                if roleType !="user" or form.data.get("department_id"):
                    ...
                    query = query.filter(~model.department_id.in_((custemer,supporter)))
