...
class InitPage:
    ...
    def _has_bill(self,user_id,date):return _BillService.HasBill(user_id, date)
    ...
    def GetRebate(self, operType):
        ...
        """
        date = dataUtil.CurrentDateStr
        if operType == 'rebate':form = _getForm("RebateForm")
        else:form = _getForm("WriteOffForm")
        flag = False
        if self.CheckFrom(form):
...
#             form.operator_id.choices = [(current_user.id,current_user.username)]
#         else:
#             choices = dataUtil.GetDataByUrl("UserChocieAll","User")()
#             choices.insert(0,(-1,""))
